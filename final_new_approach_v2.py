...
if tab == 'Home':
    ...
    # Only show department filter
    selected_department = st.selectbox('Select Department', departments, key='team_dept')

    # Job role is the same as department
    selected_role = selected_department
    tasks_for_role = roles_tasks[selected_role]
    st.subheader(f'Department: {selected_department} | Role: {selected_role}')
    st.write('**Role Tasks:**', ', '.join(tasks_for_role))

    # Multi-select employees in department
    selected_employees = st.multiselect(
        'Select up to 3 Employees',
        df_employees[df_employees['Department'] == selected_department]['Employee'],
        default=None, max_selections=3, key='team_emp'
    )

    # Radar chart and tables (unchanged from before)
    ...
    # Table: Best-fit employees for this role/department
    ...
    fit_table = df_employees[['Employee', 'Department', f'{selected_role}_Fit%'] + tasks_for_role]
    fit_table = fit_table[fit_table['Department'] == selected_department].sort_values(f'{selected_role}_Fit%', ascending=False).reset_index(drop=True)
